...
def generate_quad_candidates(in_poly):
    ...
    for indices in product(range(len_poly), repeat=4):
        ...
        if (np.sum(np.isnan(xis)) + np.sum(np.isnan(yis))) > 0:
            ...
            quad = SimplePolygon([(xis[0], yis[0]),
                                 (xis[1], yis[1]),
                                 (xis[2], yis[2]),
                                 (xis[3], yis[3])])
...
def quad_corner_diff(hull_poly, bquad_poly, region_size=0.9):
    ...
    for i in range(len(interior_points[:, 0])):
        ...
        if len(intersections) >= 2:
            corner_area_polys.append(SimplePolygon([
                intersections[0],
                intersections[1],
...
def polygon_form_factor(poly):
    ...
    """
    d_0 = np.amin(np.sqrt(np.sum(np.diff(poly.coords, axis=0) ** 2., axis=1)))
    return poly.area / (poly.length * d_0)


def detect_card_clahe(image_path, display=True, show_steps=True) -> tuple[Optional[SimplePolygon], Optional[np.ndarray]]:
    ...
    for card_contour in contours:
        ...
        if bounding_poly is None: continue

        qc_diff = quad_corner_diff(phull, bounding_poly)
        is_card_candidate = bool(
            0.001 < bounding_poly.area < image_area * 0.99 and
            qc_diff < 0.35 and
